...
## App React de revue hebdomadaire

Le script écrit un historique cumulatif (fenêtre glissante de 120 jours) dans
`latest_digest.json`, commité automatiquement par le workflow à chaque exécution.

L'app React fournie séparément (`WeeklyReview.jsx`) peut lire ce fichier
**automatiquement**, à chaque ouverture, via :
```
https://raw.githubusercontent.com/<owner>/<repo>/<branche>/latest_digest.json
```
Dans l'app, cliquez sur "Connecter un dépôt GitHub" et renseignez propriétaire /
dépôt / branche / chemin. Aucune autre action requise ensuite : à chaque ouverture
de l'app, elle relit le fichier et n'ajoute que les nouveautés.

⚠️ Cela nécessite que le dépôt soit **public** (raw.githubusercontent.com n'est
accessible sans authentification que pour les dépôts publics). Le contenu suivi
ici (CVE, bulletins, releases) est de toute façon public par nature.
